...
export function Gallery() {
  ...
  return (
    <>
      <section className="flex flex-col items-center gap-4 py-8 md:py-16 md:gap-7">
        ...
        <p className="text-lg text-muted-foreground text-balance max-w-2xl text-center">
          Some delicious food from our kitchen.
        </p>
        <div className="w-full px-8 py-4 md:py-4 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2 max-w-7xl mx-auto relative md:h-screen">
          {images.map((image: GalleryImage, index: number) => (
            <div
              key={index}
              className={`relative overflow-hidden rounded-sm md:col-span-1 lg:col-span-1 min-h-[12rem] md:min-h-0 ${image.rowSpan} cursor-pointer hover:scale-105 transition-all duration-300`}
